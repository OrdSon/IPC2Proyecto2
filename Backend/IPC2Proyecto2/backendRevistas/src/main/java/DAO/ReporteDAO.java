...
"INNER JOIN usuario as u ON u.codigo = r.usuario_codigo INNER JOIN revista j ON j.codigo = n.revista_codigo WHERE j.autor = ? AND j.codigo = ? AND r.fecha BETWEEN ? AND ?";
    
    
    public List<CommentReport> ReporteComentarios(SolicitudReporte solicitudComentarios) {
    ...
    public List<CommentReport> ReporteComentariosrRevista(SolicitudReporte solicitudComentarios) {
            ArrayList<CommentReport> registros = new ArrayList<>();

        try {
            PreparedStatement preparedStatement = connection.prepareStatement(REPORTE_COMENTARIOS_REVISTA);
            preparedStatement.setInt(1, solicitudComentarios.getAutor());
